local({
  config <- purrr::map(rlang::set_names(c("gitlab_url", "gitlab_project_id", "gitlab_private_token", "gitlab_deploy_job")),
  ...
  gitlab_manual_job(config$gitlab_url, config$gitlab_project_id, config$gitlab_deploy_job, config$gitlab_private_token)
})