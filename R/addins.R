...
gitlab_manual_job <- function(gitlab_url, private_token, project_id, deploy_jobname) {
  ...
  ci_pipeline <- gitlab_joblist(gitlab_url, private_token, project_id)
  ...
  my_handle <- gitlab_handle(private_token)
...
gitlab_projects <- function(gitlab_url, private_token) {
  resp <- glue::glue("{gitlab_url}/api/v4/projects?membership=true") %>%
...
gitlab_joblist <- function(gitlab_url, private_token, project_id) {
...
expect_status <- function(resp, status_code) {
  if (resp[["status_code"]] != status_code) {
    stop(with(gitlab_status(resp[["status_code"]]),
